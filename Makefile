...
chat: ## Chat test via LiteLLM (OpenAI-compatible)
	...
	@curl -sS http://localhost:4000/v1/chat/completions \
		...
		-d '{"model":"phi3","messages":[{"role":"user","content":"Hello! Sebutkan 3 ibu kota ASEAN beserta negaranya, dalam format JSON."}],"max_tokens":200}' \
...
chat-raw: ## Chat test directly to Ollama (bypass LiteLLM)
	...
	@curl -sS http://localhost:11434/api/generate \
		-d '{"model":"phi3:mini","prompt":"Hello, who are you?","stream":false}' \
...
chat-app: ## Chat test via FastAPI app
	...
	@curl -sS http://localhost:8080/v1/chat/completions \
		-H "Content-Type: application/json" \
		-d '{"model":"phi3","messages":[{"role":"user","content":"Apa itu machine learning? Jawab dalam 2 kalimat."}],"max_tokens":100}' \
		2>&1 | python3 -m json.tool 2>/dev/null || echo "(raw response)"
